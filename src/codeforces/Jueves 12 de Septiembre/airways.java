...
public class airways 
{
	...
	static class Edge implements Comparable <Edge>
	{
		...
		{
			from = getNode(sc.next());
			from.myEdges.add(this);
			to = getNode(sc.next());
			to.parents++;
			id = sc.nextInt();
		}
	...
	public static void main(String[] args)
	{
		...
		for(int i = 0; i < n; i++)
		{
			int f = sc.nextInt();
			nodes.clear();
			Edge[] allEdges = sc.nextObjectArray(Edge.class, f);
			...
			for(Node node : nodes.values())
			{
				if(node.parents == 0)
				{
					openEdges.addAll(node.myEdges);
					node.processed = true;
				}
			}
			while(!openEdges.isEmpty())
			{
				Edge e = openEdges.pollFirst();
				answer.add(e);
				e.to.parents--;
				if(e.to.parents == 0)
				{
					openEdges.addAll(e.to.myEdges);
					e.to.processed = true;
				}
			}
			int[] ans = new int[answer.size()];
			for(int j = 0; j < answer.size(); j++)
				ans[j] = answer.get(j).id;
			sc.printLine(ans);
		}
	}
}