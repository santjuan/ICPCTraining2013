...
public class CodeH 
{
	...
	static class ShortestPaths
	{
		...
		Object dijkstra(int desde, int hasta, boolean todos)
		{
			...
			return new Object[]{resultado[hasta], padre};
		}
	...
	public static void main(String[] args)
	{
		...
		for(int i = 0; i < m; i++) 
		{
			...
			shortest.agregarArista(a - 1, b - 1, cost);
			shortest.agregarArista(b - 1, a - 1, cost);
		}
		Object[] result = (Object[]) shortest.dijkstra(0, n - 1, true);
		long resultado = ((Long) result[0]).longValue();
		if(resultado >= ShortestPaths.INFINITO)
			System.out.println("-1");
		else
		{
			ArrayList <Integer> ruta = new ArrayList <Integer> ();
			int actual = n - 1;
			int[] padres = (int[]) result[1];
			while(actual != -1)
			{
				ruta.add(actual);
