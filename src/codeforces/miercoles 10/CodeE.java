...
public class CodeE 
{
	...
	static int cuentaTotal;
	static boolean[] isBad;
	
	static void construir(Pair currentPair, int size, int bad)
	{
		if(bad > k)
			return;
		for(char i = 'a'; i <= 'z'; i++)
		{
			long cuenta = suffix.countNormal(i, size, currentPair.a, currentPair.b);
			int badCount = bad + (isBad[i - 'a'] ? 1 : 0);
			if(cuenta != 0 && badCount <= k)
			{
				 cuentaTotal++;
				 construir(suffix.darPar(i, size, currentPair.a, currentPair.b), size + 1, badCount);
			}
		}
	}
	...
	public static void main(String[] args)
	{
		...
		String bads = sc.next();
		isBad = new boolean[bads.length()];
		for(int i = 0; i < isBad.length; i++)
			isBad[i] = bads.charAt(i) != '1';
		suffix = new SuffixArray(entrada);
		k = sc.nextInt();
		construir(new Pair(0, entrada.length() - 1), 0, 0);
