...
public class CodeF 
{
	...
	static boolean intentar(int size, int x)
	{
		...
		new SimpleSelect<IntVar>(todas.toArray(new IntVar[0]), new MostConstrainedDynamic<IntVar> (), null, new IndomainMin <IntVar> ());
		search.setOptimize(true);
		return store.consistency() && search.labeling(store, select);
	}
