...
public class lawrence 
{
	...
	{
		if(dpValue[a][b] != -1) return dpValue[a][b];
		if(a == b)
			return dpValue[a][b] = 0;
		return dpValue[a][b] = value[b] * suma(a, b - 1) + dpValue(a, b - 1);
	}
	...
	{
		if(a == n) return 0;
		if(k == 0) return dpValue(a, n - 1);
		if(dp[a][k] != -1) return dp[a][k];
		int best = Integer.MAX_VALUE;
		for(int i = a; i < n; i++)
			best = Math.min(dpValue(a, i) + dp(i + 1, k - 1), best);
		return dp[a][k] = best;
	}
	...
	public static void main(String[] args)
	{
		...
		while(true)
		{
			...
			dpValue = Scanner.fill(new int[n + 1][n + 1], -1);
