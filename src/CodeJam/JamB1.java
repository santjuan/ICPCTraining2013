...
public class JamB1 
{
	...
	public static void main(String[] args) throws FileNotFoundException
	{
		...
		System.setOut(new PrintStream("Btestt.out"));
		...
		for(int caso = 1; caso <= tc; caso++)	
		{
			long n = sc.nextLong();
			long p = sc.nextLong();
			long peor = menorRapido(n, p);
			long mejor = mayorRapido(n, p);
			System.out.println("Case #" + caso + ": " + peor + " " + mejor);
		}
	}
	
	static int mayor(int n, int p)
	{
		int[] orden = new int[1 << n];
		for(int i = 0; i < orden.length; i++)
			orden[i] = orden.length - 1 - i;
		return mayor(orden, simular(n, orden), p);
	}
	
	static long menorRapido(long n, long p)
	{
		if(p == (1L << n))
			return (1L << n) - 1;
		long bitActual = 1L << (n - 1);
		long cuentaActual = 0;
		long actual = 0;
		long aPoner = 2;
		while(true)
		{
			long siguiente = cuentaActual | bitActual;
			if(p <= siguiente)
				return actual;
			cuentaActual = siguiente;
			actual |= aPoner;
			aPoner <<= 1;
			bitActual >>= 1;
		}
	...
	static long mayorRapido(long n, long p)
	{
		long bitActual = 1L << (n - 1);
		long resultado = 0;
		for(int i = 0; i < n; i++)
