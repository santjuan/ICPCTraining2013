...
public class aliens
{
	...
	//Example with Range Maximum Query
		static class SegmentTree
		{
			...
			void update(int node, int b, int e, int pos, long value)
			{
				...
				M[node] = Math.max(M[2 * node], M[2 * node + 1]);
			}
	...
	public static void main(String[] args)
	{
		...
		while(true)
		{
			...
			SuffixArray suffix = new SuffixArray(entrada);
			SegmentTree segment = new SegmentTree(entrada.length());
			for(int i = 0; i < entrada.length(); i++)
				segment.update(1, 0, entrada.length() - 1, i, suffix.sa[i]);
			int bestLen = -1;
			int bestRight = 0;
			for(int i = 0; i + m <= entrada.length(); i++)
			{
				int lcp = suffix.lcp(i, i + m - 1);
				if(lcp > 0)
					if(bestLen <= lcp)
					{
						int rightMost = (int) segment.query(1, 0, entrada.length() - 1, i, i + m - 1);
						if(bestLen == lcp && bestRight > rightMost) continue;
						bestLen = lcp;
						bestRight = rightMost;
					}
			}
			if(bestLen == -1) System.out.println("none");
			else System.out.println(bestLen + " " + bestRight);
		}
	}
}
